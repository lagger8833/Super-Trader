...
def _setup_logging():
    ...
    # Print current public IP — this must match the IP whitelisted in
    # mStock API settings (trade.mstock.com → Trading APIs → API Settings)
    try:
        ...
        logger.info(">>> If API calls fail with 'IP Address not matching',")
        logger.info(">>> add %s to your mStock API whitelist.", public_ip)
    except Exception as e:
        logger.info("Current public IP : could not fetch (%s)", e)

    try:
        import socket
        local_ip = socket.gethostbyname(socket.gethostname())
        logger.info("Local network IP  : %s", local_ip)
    except Exception:
        pass
