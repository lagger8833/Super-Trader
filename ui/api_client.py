...
def _parse_response(resp) -> dict:
    """
    Extract JSON from SDK response or raw requests.Response.
    The API sometimes returns a list (e.g. place_order success returns
    [{"order_id": "..."}]).  We normalise that to {"data": <list>}
    so the rest of the code always works with a dict.
    """
    try:
